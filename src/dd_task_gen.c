...
void periodicTaskGenerator1 (void *pvParameters)
{
	...
	while(1)
	{
		xTaskCreate(vPeriodicTask1, "PeriodicTask1", configMINIMAL_STACK_SIZE, NULL, IDLE_TASK_PRIORITY, &xTaskHandle1); // create task
...
void periodicTaskGenerator2 (void *pvParameters)
{
	...
	while(1)
	{
		xTaskCreate(vPeriodicTask2, "PeriodicTask2", configMINIMAL_STACK_SIZE, NULL, IDLE_TASK_PRIORITY, &xTaskHandle2); // create task
